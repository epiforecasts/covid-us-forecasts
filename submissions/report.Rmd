...
if (exists("anomalies")) {
data_anomalous <- plot_data(state %>% 
                              filter(state %in% anomalies))
}
```
...
#### Anomalous
```{r plot-data-anomalous, echo=FALSE, fig.align = 'center', out.width="100%", warning=FALSE, message=FALSE}
if (exists("data_anomalous")) {
  plot(data_anomalous)
}
```
...
```{r plot-models-anomalous, echo=FALSE, fig.align = 'center', out.width="100%", out.height="100%"}
if (exists("anomalies")) {
  knitr::include_graphics(here::here("evaluation", "plots", target_date,
                                     "models", "anomalies.png"))
}
```
...
```{r plot-ensembles-anomalous, echo=FALSE, fig.align = 'center', out.width="100%", out.height="100%"}
if (exists("anomalies")) {
  knitr::include_graphics(here::here("evaluation", "plots", target_date,
                                     "ensembles", "anomalies.png"))
}
```
...
print(errors)
```
The QRA is submitted by default. Change this for any combination of states/models using `submissions/utils/swap-ensembles.R`. Specify state/model combinations in the `swap` list, then source the script.
