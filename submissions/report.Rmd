...
errors <- readRDS(here("submissions", "submission-notes", paste0(target_date, "-errors.rds")))
print(errors)
```
...
## Submission
The QRA is submitted by default. Change this for any combination of states/models using [`submissions/swap-ensembles.R`](https://github.com/epiforecasts/covid-us-forecasts/blob/master/submissions/swap-ensembles.R). Specify state/model combinations in the `swap` list, then source the script.
